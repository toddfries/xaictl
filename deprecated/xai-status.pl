...
my $CLIENT_VER    = 'xai-status-pl/1.0';
...
my $force_refresh = 0;
my $no_refresh    = 0;
my $refresh_only  = 0;   # just refresh + print auth refresh keys, exit
my $history_max_sessions = 200;  # cap per-session dump lines

GetOptions(
    'section|s=s@' => \@want_sections,
    'json|j'       => \$json_raw,
    'auth|a=s'     => \$auth_path,
    'base|b=s'     => \$proxy_base,
    'timeout|t=i'  => \$timeout,
    'list|l'       => \$list,
    'refresh|r'    => \$force_refresh,
    'no-refresh'   => \$no_refresh,
    'refresh-only' => \$refresh_only,
    'history-max-sessions=i' => \$history_max_sessions,
    'help|h'       => \$help,
) or usage(2);

die "cannot combine --refresh and --no-refresh\n" if $force_refresh && $no_refresh;
$force_refresh = 1 if $refresh_only;
...
if (!@want_sections) {
    @want_sections = qw(auth user subscription credits legacy autotopup settings models);
}
for my $s (@want_sections) {
    die "unknown section '$s' (try -l)\n" unless $SECTIONS{$s} || $s eq 'billing';
}
# "billing" is an alias for both credit shapes
@want_sections = map {
    $_ eq 'billing' ? (qw(credits legacy)) : $_
} @want_sections;

# ---------------------------------------------------------------------------
# Auth + HTTP
# ---------------------------------------------------------------------------
my $grok_home = $ENV{GROK_HOME}
  // File::Spec->catdir($ENV{HOME} // die("HOME unset\n"), '.grok');
$auth_path //= $ENV{XAI_STATUS_AUTH}
  // $ENV{GROK_AUTH_PATH}
  // File::Spec->catfile($grok_home, 'auth.json');
$proxy_base //= $ENV{GROK_CLI_CHAT_PROXY_BASE_URL}
  // $ENV{GROK_PRODUCTION_CLI_CHAT_PROXY_BASE_URL}
  // $DEFAULT_PROXY;
$proxy_base =~ s{/*$}{};

my ($auth, $scope) = load_auth($auth_path);
...
# Auto-refresh when expired / within early-invalidation window, or on --refresh.
# Writes the new tokens back to auth.json (same as Grok's AuthManager::update).
my $refresh_info;
if (!$no_refresh) {
    my $need = $force_refresh || token_needs_refresh($auth);
    if ($need) {
        $refresh_info = refresh_and_persist($auth_path, $scope, $auth);
        if ($refresh_info && $refresh_info->{ok} && $refresh_info->{auth}) {
            $auth = $refresh_info->{auth};
        }
    }
}
...
if ($refresh_only) {
    ...
    emit_refresh_section($refresh_info, $auth, $scope, $auth_path);
    exit($refresh_info && $refresh_info->{ok} ? 0 : 1);
}

emit_auth_section($auth, $scope, $auth_path, $proxy_base)
  if grep { $_ eq 'auth' } @want_sections;
emit_refresh_section($refresh_info, $auth, $scope, $auth_path)
  if $refresh_info;

my %done;
for my $sec (@want_sections) {
    next if $sec eq 'auth';
    next if $done{$sec}++;
    if ($sec eq 'user') {
...
sub kv {
    ...
    # stringify carefully (no newlines)
    $val = "$val";
    $val =~ s/\r//g;
    $val =~ s/\n/\\n/g;
    print "$key=$val\n";
...
sub kv_date {
    ...
    kv("$key.raw", $raw);
    my $local = $opt{fast}
      ? format_date_local_fast($raw)
      : format_date_local($raw);
    $local //= format_date_local_fast($raw);
    kv($key, defined $local ? $local : $raw);
    my $epoch = parse_rfc3339($raw);
    kv("$key.unix", $epoch) if defined $epoch;
}
...
sub usage {
    my ($code) = @_;
    print <<'EOF';
Usage: xai-status.pl [options]

Dump xAI Grok Build account status (credits, subscription, user, settings)
as sysctl-style key=value lines. Uses the same cli-chat-proxy endpoints and
~/.grok/auth.json credentials as Grok Build.

Options:
  -s, --section NAME   Limit to section (repeatable). Default: live status
                       (not history). Sections: auth user subscription
                       credits legacy autotopup settings models history
                       Alias: billing = credits + legacy
  -r, --refresh        Force OIDC refresh (writes auth.json)
      --no-refresh     Never call the IdP / never rewrite auth.json
      --refresh-only   Refresh (or adopt sibling token), print refresh
                       keys, exit (no billing/status fetches)
      --history-max-sessions N
                       Cap per-session dump under history (default 200)
  -j, --json           Also emit compact raw JSON per section (*.json=...)
  -a, --auth PATH      Path to auth.json (default: ~/.grok/auth.json)
  -b, --base URL       cli-chat-proxy base (default: https://cli-chat-proxy.grok.com/v1)
  -t, --timeout SECS   HTTP timeout (default: 20)
  -l, --list           List sections and exit
  -h, --help           This help

By default the script refreshes when the access token is expired or within
300s of expiry (same early-invalidation buffer as Grok), and always writes
the new tokens to auth.json under auth.json.lock.

The history section mines server monthly dollar rows, ~/.grok/logs/unified.jsonl
billing polls (prepaid/% timeline, day/hour buckets, fast burns), and local
session summaries/signals. There is still no server tokens-per-day API.

Examples:
  xai-status.pl | grep '^xai.credits'
  xai-status.pl -s credits -s autotopup
  xai-status.pl --refresh-only          # renew session like another Grok instance
  xai-status.pl -s subscription -s credits | egrep 'tier|usage|prepaid|remaining'
  xai-status.pl --no-refresh -s history | egrep 'monthly|prepaid|by_day|fast_burn|timeline'
  xai-status.pl --no-refresh -s history > /tmp/xai-history.txt

Docs:
  man ./scripts/xai-status.1            # or: mandoc -a scripts/xai-status.1
  less scripts/xai-status-api.md        # formal API for re-implementers
EOF
    exit $code;
}
